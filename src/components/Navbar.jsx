    ...
    <nav className=" sticky top-0 left-0 z-30 border-gray-200 bg-white dark:bg-gray-900">
      <div className="flex items-center justify-around mx-auto py-4 px-2 w-[100vw] text-white">
        ...
        <div className="flex items-center space-x-1 rtl:space-x-reverse">
          ...
          <span className="self-center text-2xl max-md:text-xl max-md:-ml-2 font-semibold whitespace-nowrap text-[var(--secondary)]">
            Shoppire
          </span>
        </div>
        ...
        <div className="flex items-center gap-5 max-md:gap-2 md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse relative">
          ...
          <div className="relative">
            ...
            <p className=" bg-[var(--secondary)] absolute -top-3.5 left-[2.5px] rounded-full h-5 w-5 text-center text-sm">
              {cart}
            </p>
          </div>
          ...
          {!localStorage.getItem("token") && (
            <button
              ...
              className="btn bg-[var(--secondary)] py-2 px-5"
            >
              Login
            </button>
          )}
          ...
          {localStorage.getItem("token") && (
            <img
              src="imgs/admin/profile_img_3.png"
              alt="user img"
              onClick={() => setUserMenuShow((show) => !show)}
              className="text-gray-900 text-xl cursor-pointer dark:text-white w-9"
            />
          )}
          ...
          {localStorage.getItem("token") && (
            <div
              className={`${userMenuShow ? "absolute" : "hidden"} z-10 top-10 right-0 md:-right-10 md:top-9 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow-sm dark:bg-gray-700 dark:divide-gray-600`}
              id="user-dropdown"
            >
          ...
          <button
            onClick={() => setToggleMenuShow((show) => !show)}
        ...
        {/*  */}
        <div
          className={`items-center justify-between w-full md:flex md:w-auto md:order-1  max-md:absolute max-md:top-19 max-md:bg-white max-md:w-[99%] rounded-md z-20 ${toggleMenuShow ? "flex" : "hidden"}`}
          id="navbar-user"
        >
          <ul className="flex flex-col font-medium p-4 md:p-0 mt-4 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0">
            <li>
              <NavLink
                to="/"
                className={({ isActive }) =>
                  `py-2 px-3 text-[var(--secondary)] dark:text-[var(--secondary)] flex items-center gap-2 ${isActive ? " " : "text-gray-900 dark:text-white"}`
                }
              >
                {" "}
                <SiGooglehome /> Home
            ...
            <li>
              <NavLink
                to="collection"
                className={({ isActive }) =>
                  `flex items-center py-2 px-3 text-[var(--secondary)] dark:text-[var(--secondary)] gap-2 ${isActive ? "" : "text-gray-900 dark:text-white"}`
                }
              >
                {" "}
                <BsCollectionFill /> Collection
              </NavLink>
            </li>
            ...
            <li className="flex items-center">
              <a
                href="#footer"
                className=" ` flex items-center py-2 px-3 text-gray-900 gap-2 dark:text-white"
              >
                {" "}
                <SiMaildotcom /> Contact
              </a>
            </li>
            {/* <li>
